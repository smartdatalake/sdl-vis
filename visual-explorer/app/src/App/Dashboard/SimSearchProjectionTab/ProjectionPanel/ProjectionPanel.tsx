...
const ProjectionPanel = () => {
    const [algorithm, setAlgorithm] = useState<ProjectionAlgorithm>(ProjectionAlgorithm.MDS);
    const {
        isLoading,
    ...
    const activateAttribute = (searchParameter: SearchParameterValue) => {
        searchParameter.active = true;
        searchParameter.value = searchParameter.sampleValue;
            ...
            {!isMinimized && (
                <Card.Body>
                    <Grid>
                        ...
                        <div>
                            ...
                            <IndentedParagraph>
                                <Grid>
                                    <KEquals>k =</KEquals>
                                    <Form.Control defaultValue={'15'} name={'k'} onBlur={onKChanged} type="text" />
                                </Grid>
                            </IndentedParagraph>
                        </div>
                    </Grid>
                    ...
                    <IndentedParagraphWithoutTopMargin>
                        {searchParameters
                            ? Object.entries(searchParameters)
                                  ...
                                  .map(([name, value], index) => {
                                      return (
                                          <SearchParameter key={name} parameterName={name} searchParameter={value} />
                                      );
                                  })
                            : null}
                        <DropDownButton>
                            <Dropdown.Toggle id="dropdown-basic-button" block={true}>
                                Add Attribute
                            </Dropdown.Toggle>
                            <DropDownMenu>
                                {searchParameters
                                    ? Object.entries(searchParameters)
                                          .filter(([name, value]) => !value.active)
                                          .map(([name, value], index) => {
                                              const onClickHandler = () => activateAttribute(value);

                                              return (
                                                  <DropDownItem key={name} onClick={onClickHandler}>
                                                      <div> {name} </div>
                        ...
                        <QueryButton
                            onClick={() => {
                                //TODO: check if attributes are defined and correct type is used
