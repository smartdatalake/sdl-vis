...
class BarchartCumulative {
  ...
  constructor(selector, distributionTop, distributionBottom, properties, histogramfunctions) {
    ...
    for(let i=0; i<histogramTop.length; i++){
      ...
      let x0 = histogramTop[i].x0;
      let x1 = histogramTop[i].x1;

      histogramTop[i] = histogramTop[i].concat(lastTop);
      histogramBottom[i] = histogramBottom[i].concat(lastBottom);
      histogramTop[i].x0 = x0;
      histogramBottom[i].x0 = x0;
      histogramTop[i].x1 = x1;
      histogramBottom[i].x1 = x1;

      lastTop = histogramTop[i];
